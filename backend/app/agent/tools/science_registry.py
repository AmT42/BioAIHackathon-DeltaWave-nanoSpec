...
def create_science_registry(settings: Settings) -> ToolRegistry:
    ...
    if settings.enable_literature_tools and (settings.enable_pubmed_tools or settings.enable_openalex_tools):
        if settings.enable_openalex_tools and not settings.openalex_api_key:
            logger.warning("OpenAlex tools enabled but OPENALEX_API_KEY is missing; OpenAlex tools will be omitted.")
    ...
    if settings.enable_optional_source_tools:
        if not settings.epistemonikos_api_key:
            logger.warning(
                "Epistemonikos API key is missing; Epistemonikos tools will be omitted from optional sources."
            )
        tools.extend(build_optional_source_tools(settings, http))
    tools.extend(build_evidence_tools())
    tools.extend(build_pipeline_tools(settings, http))
