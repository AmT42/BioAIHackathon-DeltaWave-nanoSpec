...
class AgentSettings(BaseModel):
    ...
    search_count: int = 8
    external_search_provider: Literal["openalex", "local"] | None = Field(
        default=None,
        description=(
            "Optional external search provider for the paper_search tool. "
            "Set to 'openalex' to fetch papers from OpenAlex before local indexing."
        ),
    )  #OA hp
    external_search_max_results: int = Field(
        ...
        ge=10,
        description=(
            "Maximum number of OpenAlex hits to consider per paper_search call "
            "before filtering to the requested count."
        ),
    )  #OA hp
    external_pdf_max_downloads: int | None = Field(
        default=None,
        description=(
            "Upper bound on the number of PDFs to download per paper_search call "
            "when using an external provider. Defaults to `search_count`."
        ),
    )  #OA hp
    require_open_access: bool = Field(
        default=True,
        description=(
            "When True, restrict OpenAlex search results to is_oa:true works at query"
            " time."
        ),
    )
    prefer_fulltext_order: list[str] = Field(
        default_factory=lambda: ["pdf", "jats", "html"],
        description=(
            "Preferred order of full-text formats to ingest when multiple options are"
            " available."
        ),
    )
    allow_bronze: bool = Field(
        default=False,
        description=(
            "Allow ingestion of Bronze open access works which may not carry an"
            " explicit license."
        ),
    )
    html_ingest_enabled: bool = Field(
        default=True,
        description="Enable ingestion of HTML full-text content when available.",
    )
    jats_ingest_enabled: bool = Field(
        default=True,
        description="Enable ingestion of XML/JATS full-text content when available.",
    )
    sink_to_file: bool = Field(
        default=False,
        description=(
            "Stream confirmed PDF downloads to temporary files instead of keeping them"
            " in memory."
        ),
    )
    headless_pdf_enabled: bool = Field(
        default=False,
        description=(
            "Use a headless browser fallback (Playwright) to click download buttons"
            " when static techniques fail."
        ),
    )
    headless: bool = Field(
        default = False,
        description= 'open chromonium in gui or not'
    )
    ignore_license_filter: bool = Field(
        default=False,
        description=(
            "When True, skip license checks during full-text resolution."
            " Use with caution; non-open licenses may be ingested."
        ),
    )
    fulltext_archive_dir: str | None = Field(
        default=None,
        description=(
            "Optional directory path to store a copy of each full-text artifact "
            "ingested from OpenAlex. Leave unset to skip archiving."
        ),
    )
    collect_run_stats: bool = Field(
        default=True,
        description=(
            "When True, capture resolver telemetry for each OpenAlex-backed run "
            "and write summaries to disk."
        ),
    )
    run_stats_dir: str = Field(
        default="data",
        description="Base directory to store OpenAlex run statistics.",
    )
    run_id: str | None = Field(
        default=None,
        description="Optional identifier for OpenAlex run statistics output directories.",
    )
    allow_domains: list[str] = Field(
        ...
        description="Domains explicitly allowed for full-text downloads.",
    )
    block_domains: list[str] = Field(
        ...
        description="Domains to avoid when downloading full-text content.",
    )
    respect_robots_txt: bool = Field(
        default=True,
        description="Respect robots.txt directives for candidate download URLs.",
    )
    http_per_host_rps: float = Field(
        default=8.0,
        description="Maximum requests-per-second budget per host for HTTP downloads.",
    )
    http_max_concurrent_downloads: int = Field(
        default=6,
        ge=1,
        description="Maximum number of concurrent full-text downloads in progress.",
    )
    http_timeout_s: float = Field(
        default=30.0,
        description="Default HTTP timeout for OpenAlex and full-text fetch requests.",
    )
    http_stream_peek_bytes: int = Field(
        default=2048,
        ge=256,
        description="Number of bytes to peek from a stream to guess its content type.",
    )
    max_pdf_mb: int = Field(
        default=80,
        ge=1,
        description="Skip PDFs larger than this size (in megabytes).",
    )
    max_html_mb: int = Field(
        default=20,
        ge=1,
        description="Skip HTML/JATS downloads larger than this size (in megabytes).",
    )
    per_work_resolution_budget_s: float = Field(
        default=50.0,
        description="Total time budget for resolving full-text for a single work.",
    )
    per_link_timeout_s: float = Field(
        default=8.0,
        description="Timeout for probing any individual candidate link.",
    )
...
class Settings(BaseSettings):
    ...
    def make_aviary_tool_selector(self, agent_type: str | type) -> ToolSelector | None:
        ...
        if agent_type is ToolSelector or (
            ...
            router = None
            get_router = getattr(agent_llm, "get_router", None)
            if callable(get_router):
                router = get_router()
            else:
                router = getattr(agent_llm, "router", None)
            if router is None or not hasattr(router, "acompletion"):
                raise AttributeError(
                    "LiteLLMModel must expose router.acompletion (or get_router().acompletion)."
                )
            return ToolSelector(
