...
def get_openalex_mailto() -> str | None:
    """Get the OpenAlex mailto address from environment, if available; otherwise None."""
    mailto_address = os.getenv("OPENALEX_MAILTO")
    if not mailto_address:
        logger.warning(
...
def _normalize_doi(value: str | None) -> str | None:
    ...
    Converts to lowercase and strips common URL prefixes so that
    "10.xxxx/yyy" and "https://doi.org/10.xxxx/yyy" compare equal.
    """
    if not value:
        return value
    v = value.strip().lower()
...
def _extract_response_doi(data: dict[str, Any]) -> str | None:
    """Extract DOI from an OpenAlex work payload.

    Prefers the top-level "doi" field, falling back to ids.doi if needed.
    Returns the raw value (may be a URL); caller should normalize.
    """
    doi = data.get("doi")
    if not doi and isinstance(data.get("ids"), dict):
...
async def get_doc_details_from_openalex(  # noqa: PLR0912
    ...
    if doi:
        resp_doi = _extract_response_doi(results_data)
        if _normalize_doi(resp_doi) != _normalize_doi(doi):
