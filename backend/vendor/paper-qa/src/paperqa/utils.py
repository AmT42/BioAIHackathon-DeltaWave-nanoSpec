...
def citation_to_docname(citation: str) -> str:
    ...
    # Prefer title-like tokens to preserve existing naming behavior.
    match = re.search(r"([A-Z][a-z]+)", citation)
    if match is not None:
        author = match.group(1)
    else:
        # Fallback for acronym-heavy titles (e.g. "CD47/SIRPα ...").
        acronym_match = re.search(r"([A-Z0-9]{2,})", citation)
        if acronym_match is not None:
            author = acronym_match.group(1)
        else:
            # Last resort: deterministic, parse-safe identifier.
